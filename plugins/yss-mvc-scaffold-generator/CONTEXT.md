...
| 术语 | 定义 | 英文标识 | 避免 / 备注 |
|---|---|---|---|
| 分析主题 | 面向业务目标组织的一组数据表、指标、维度和查询模板。 | AnalysisTopic | 不使用“报表主题” |
| 数据表说明 | 以 Markdown 维护的 Oracle 表结构契约，不承载运行时数据。 | DataTableSpecification | 不使用“数据源文件” |
| 分析查询 | 用户提交的只读 SQL，仅允许 SELECT/CTE 及 Oracle 分析语法。 | AnalysisQuery | 不使用“自由脚本” |
| 查询执行 | 使用 Oracle 数据源执行通过权限、只读、超时和行数校验的查询。 | QueryExecution | 不使用“直接执行 SQL” |
| 分析结果 | 分页明细、聚合指标、图表配置及 Excel/CSV 导出数据。 | AnalysisResult | 不使用“原始结果集” |
| 发布确认 | 自动门禁通过后的一次业务负责人确认。 | ReleaseConfirmation | 不使用“多级审批” |
