...
项目实例可以只提交 `skills-lock.json`，把完整技能安装到 Agent 的用户级目录，再执行环境检查：

```bash
node scripts/check-agent-environment.mjs --agent=codex
```

检查器只读取项目锁文件和用户级技能目录，校验技能是否存在及 `effectiveHash` 是否一致。失败时返回 `NOT_READY` 并停止后续流程；不会自动下载、覆盖或删除技能。其他 Agent 可将 `--agent` 改为 `claude`、`cursor`、`hermes`、`pi`、`qoder` 或 `trae`。
